...
int32_t sin_fixed(int32_t th_rad_fxp)
{
	 ...
	 */
	static const int32_t cordicsine_lookup[31] = { 843314857, 497837829, 263043837, 133525159,
	...
	int32_t cdc_sin_th;
	int sign;
	/*1073741823, deg = +57.2958*/
	const int32_t CORDICSINE_P57DEG_Q30FL = Q_CONVERT_FLOAT(1.99999999813735, 29.9999999986564);
	...
	/*-1073741824, deg = -57.2958*/
	/* Minor-Bug,-1073741824 = round(-1*2^30) but QCF is -1073741823*/
	const int32_t CORDICSINE_N57DEG_Q30FL = Q_CONVERT_FLOAT(-1, 30);
	...
	if (sign == -1)
		...
		th_rad_fxp = sign * b_yn;
	/*To support Q1.31 format, look for +/-57deg*/
	if (th_rad_fxp > CORDICSINE_P57DEG_Q30FL)
		th_rad_fxp = INT32_MAX;
	else if (th_rad_fxp <= CORDICSINE_N57DEG_Q30FL)
		th_rad_fxp = INT32_MIN;
	else
		th_rad_fxp <<= 1;

	cdc_sin_th = th_rad_fxp;
	return cdc_sin_th;
